...
$$
which will be used to define the associated **multi-level graph** $\mathcal{M}(G;S_1,\dots,S_l)$ with its $l+1$ levels.
...
Supposing we just built level $L_{i-1}$, we now consider for each node $u\in S_{i-1}$ a shortest-path tree $T_u$ in $(S_{i-1},E_{i-1})$.
